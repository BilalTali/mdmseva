...
export default function Sidebar({ currentRoute }) {
                ...
                <div className="mb-2">
                    ...
                    <div className={`overflow-hidden transition-all duration-300 ${isSchoolConfigOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
                        <div className="ml-4 mt-2 space-y-1 pl-4 border-l-2 border-[var(--border-light)]">
                            ...
                            {/* Rice Configuration with sub-actions */}
                            <div>
                                <Link
                                    href={route('rice-configuration.create')}
                                ...
                                {currentRoute && currentRoute.startsWith('rice-configuration') && (
                                    <div className="ml-6 mt-1 space-y-1 pl-3 border-l-2 border-[var(--primary-200)]">
                                        <Link
                                            href={route('rice-configuration.edit')}
                                            className="flex items-center px-3 py-1.5 text-xs text-[var(--primary-600)] hover:text-[var(--primary-800)] hover:bg-[var(--primary-50)] rounded transition-all"
                                        >
                                            <svg className="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <Link
                                            href={route('rice-configuration.index')}
                                            className="flex items-center px-3 py-1.5 text-xs text-[var(--primary-600)] hover:text-[var(--primary-800)] hover:bg-[var(--primary-50)] rounded transition-all"
                                        >
                                            <svg className="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    return (
        <>
            ...
            {/* Mobile Sidebar */}
            <aside
                className={`lg:hidden fixed top-0 left-0 z-40 w-72 min-h-screen flex flex-col transform transition-transform duration-300 ease-in-out shadow-2xl bg-[var(--sidebar-bg)] ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                <SidebarContent />
            </aside>
