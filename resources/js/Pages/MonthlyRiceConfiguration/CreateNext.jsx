...
export default function CreateNext({ auth, lastCompleted, nextMonth, nextYear, carriedBalance, schoolTypes, userSchoolType }) {
    const monthNames = [
    ...
    const { data, setData, post, processing, errors } = useForm({
        month: nextMonth,
        year: nextYear,
        school_type: auth.user.school_type || 'primary', // Default to user's school type
        daily_consumption_primary: lastCompleted.daily_consumption_primary, // Carry forward consumption rates
        daily_consumption_upper_primary: lastCompleted.daily_consumption_upper_primary,
        opening_balance_primary: carriedBalance.primary,
        opening_balance_upper_primary: carriedBalance.upper_primary,
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('monthly-rice-config.store-next'));
    };
    ...
    return (
            ...
            header={
                <div className="flex justify-between items-center">
                    <h2 className="font-semibold text-xl text-gray-800 leading-tight">
                        Create Rice Configuration - {monthNames[nextMonth - 1]} {nextYear}
                    </h2>
                    <Link
        ...
        >
            <Head title={`Create Configuration - ${monthNames[nextMonth - 1]} ${nextYear}`} />
            ...
            <div className="py-12">
                <div className="max-w-4xl mx-auto sm:px-6 lg:px-8">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {/* Note: School type is usually fixed for next month creation based on previous, but we allow editing if needed */}
                        <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        ...
                        <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                            ...
                            <div className="mb-4 p-4 bg-green-50 border-l-4 border-green-500 rounded-md">
                                <p className="text-sm text-green-800 font-medium mb-1">
                                    ✓ Auto-filled from {monthNames[lastCompleted.month - 1]} {lastCompleted.year} Closing Balance:
                                </p>
                                <div className="text-sm text-green-700">
                                    <span>Primary: <strong>{carriedBalance.primary} kg</strong></span>
                                    {showUpperPrimary() && (
                                        <span className="ml-4">Middle: <strong>{carriedBalance.upper_primary} kg</strong></span>
                                    )}
                                </div>
