{{-- Location: resources/views/bills/pdf.blade.php --}}
@php
...
<head>
    <meta charset="utf-8">
    <title>{{ $bill->getTypeLabel() }} - {{ $bill->getBillNumber() }}</title>
    <style>
        @page { 
            size: A4 portrait; 
        ...
        * {
        ...
        .header-top-bar {
            background: linear-gradient(90deg, {{ $colors['primary'] }} 0%, {{ $colors['secondary'] }} 50%, {{ $colors['secondary'] }} 100%);
            padding: 2px 0;
        }
    ...
    <div class="header-border">
        ...
        <div class="header-content">
            ...
            <div class="header-info {{ $headerClass }}">
                ...
                @endif
                @if(!empty($bill->shop_gstin))
                <div>
                    <strong>GSTIN</strong>
                    {{ $bill->shop_gstin }}
                </div>
                @endif
                @if(!empty($bill->shop_license_no))
                <div>
                ...
                <div>
                    ...
                    {{ strtoupper($bill->payment_mode) }}
                </div>
                @endif
                @if(!empty($bill->fuel_type))
                <div>
                    <strong>Fuel Type</strong>
                    {{ strtoupper($bill->fuel_type) }}
                </div>
                @endif
            </div>
        </div>
        <div class="invoice-badge">
            {{ strtoupper($bill->getTypeLabel()) }} #{{ $bill->getBillNumber() }}
        </div>
    </div>
    ...
    <div class="content-wrapper">
        ...
        <div class="bill-section">
            <div class="bill-to">
                ...
                <div class="info-line">
                    <strong>{{ $user->school_name ?? $user->name }}</strong>
                </div>
                @if($user->udise)
                <div class="info-line">
                    <strong>UDISE Code:</strong> {{ $user->udise }}
                </div>
                @endif
                <div class="info-line">
                    <strong>Bill For Month:</strong> {{ $report->period }}
                </div>
            ...
            <div class="invoice-info">
                ...
                @if(!empty($bill->shop_gstin))
                <div class="info-line">
                    <strong>GSTIN:</strong> {{ $bill->shop_gstin }}
                </div>
                @endif
                @if(!empty($bill->shop_license_no))
                <div class="info-line">
                    <strong>License No:</strong> {{ $bill->shop_license_no }}
                </div>
                @endif
                @if(!empty($bill->payment_mode))
                <div class="info-line">
                    <strong>Payment Mode:</strong> {{ strtoupper($bill->payment_mode) }}
                </div>
                @endif
                @if(!empty($bill->fuel_type))
                <div class="info-line">
                    <strong>Fuel Type:</strong> {{ strtoupper($bill->fuel_type) }}
                </div>
                @endif
            </div>
        </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right">₹ {{ number_format($item->rate_per_unit, 2) }}</td>
                ...
                <tr class="total-row">
                    ...
                    <td class="text-right font-bold">₹ {{ number_format($bill->total_amount, 2) }}</td>
                    <td colspan="3"></td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="amount-words-box">
            <div style="margin-bottom: 8px;">
                Amount in Words: <span style="text-transform: uppercase;">{{ ucwords(numberToWords($bill->total_amount)) }} Rupees Only</span>
            </div>
        ...
        <div class="signature-section">
            ...
            <div class="institution-verification">
                ...
                <div class="verification-fields">
                    ...
                    <div class="verification-line" style="margin-top: 8px; padding-top: 8px; border-top: 1px dashed {{ $colors['primary'] }};">
